...
if [ ! -d "/usr/share/fonts/truetype/cascadia" ]; then
    ...
    unzip /workspaces/Shenzhen_innoX_2024wc_ws/docker_init/res/cascadia-fonts.zip -d /tmp/cascadia-fonts
...
if [ "${INSTALL_NOVNC}" = "true" ] && [ ! -d "/usr/local/novnc" ]; then
    ...
    unzip /workspaces/Shenzhen_innoX_2024wc_ws/docker_init/res/noVNC-1.2.0.zip -d /usr/local/novnc
    cp /usr/local/novnc/noVNC-${NOVNC_VERSION}/vnc.html /usr/local/novnc/noVNC-${NOVNC_VERSION}/index.html

    # curl -sSL https://github.com/novnc/websockify/archive/v${WEBSOCKETIFY_VERSION}.zip -o /tmp/websockify-install.zip
    unzip /workspaces/Shenzhen_innoX_2024wc_ws/docker_init/res/websockify-install.zip -d /usr/local/novnc
